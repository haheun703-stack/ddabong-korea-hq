...
- **화면 문구** (legacy 그대로, 영문 표기는 미정 (편집 시 결정) — legacy 는 한국어 단계명만 기재):
  ...
  STEP 4  흙 봉분 완성
  ```
- **레이아웃/애니메이션**: 각 단계 0.8~1.2초. Higgsfield H01/H02/H03 (`EP01_S04_SH004`–`SH006`, `EP01_S05_SH005`) 과 연결. 설명 구간, 음악 단순 (러프컷). 16초 샷 대비 단계 합계 3.2~4.8초 → 나머지 시간 처리 미정 (편집 시 결정) (§5 #3).
