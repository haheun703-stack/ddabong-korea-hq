...
**예외 (D-058, 2026-09-19)**: `HERITAGE_INSERT` (사람 0 의 구조물 컷어웨이, 룩 `DDABONG_TECH_INFOGRAPHIC_V01`) 는 AI 영상 **컷당 ≤10초** 허용. 위 금지의 근거인 얼굴·의상 드리프트가 사람 없는 컷에는 없기 때문이다. 나머지 금지(화면 내 글자·text-to-video 시작·실제 장소 대체)는 인서트에도 그대로 적용된다. 상세 `SHOT_STANDARD.md` §HERITAGE_INSERT.
