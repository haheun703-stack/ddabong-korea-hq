...
## 0. 주제 선정 (DRAFT, 2026-09-19 · P-014-B · 근거 `09_ANALYTICS/benchmarks/WORKFLOW_MASTERPROMPT_FLOW_20260919.md` §3-B)

7단계는 주제가 정해진 뒤부터 돈다. 그 앞에 주제를 고르는 자리가 없었다. EP02 부터:

- 봇이 `CONTENT_ROADMAP.md` 범위 안에서 **후보 5개** 를 낸다. 후보마다 제목 · 한 줄 (시각 + 감정) · **3축 점수** (호기심 / 바이럴 잠재력 / 감정 영향력, 각 10점 만점 소수점 1자리) · 채널 질문 "Why is Korea like this?" 에 답할 수 있는지 한 줄.
- 점수는 봇 추정치다. **결정 보조**로만 쓰고 최종 선택은 사용자. 점수가 낮은 후보를 사용자가 고를 수 있고 그 경우 사유를 `DECISIONS.md` 에 남긴다.
- 후보를 낼 때 `BENCHMARK_STANDARD` 대상 영상 1편을 같이 제안한다 (착수 게이트 요건).
- 5개가 다 마음에 안 들면 다시 5개. 후보 이력은 `09_ANALYTICS/topics/` 에 남긴다 (신설 예정).
