...
- **운영 방식 (D-025)**: 무료 작업은 계획 → 실행 → 검수 에이전트(별도 컨텍스트) OS 규칙 점검 → 검증기 → 커밋 → push 까지 자율. 사람 관문은 ① 유료 생성 승인(Money Gate) ② 그림 최종 OK/FIX (봇이 먼저 거름) ③ 역사 해석 결정 ④ main 병합. 다른 회사 모델(Codex) 교차 검수는 큰 단계 완료 시에만.
